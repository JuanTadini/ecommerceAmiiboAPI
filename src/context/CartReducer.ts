...
export const cartReducer = (state: CartState, action: CartAction): CartState => {
  switch(action.type){
    ...
    case "REMOVE_FROM_CART": {
      const {id: removedItemID} = action.payload;

      // Validar si el item ya existe en el carrito
      const itemToRemove = state.cartItems.find((item) => item.id === removedItemID);
      ...
      if (itemToRemove) {
        if(itemToRemove.quantity === 1){
          ...
          return {
            ...state,
            cartItems: state.cartItems.map((item) => item.id === removedItemID ? {...itemToRemove, quantity: itemToRemove.quantity - 1} : item)
          }
        }
      }
